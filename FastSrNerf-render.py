...
class PostProcessor:
    ...
    def _query_density_color(self, points):
        ...
        """
        # TensoRF expects samples in shape [N, S, 3]. Here S=1 since we query a single point at a time.
        N = points.shape[0]
...
if __name__ == "__main__":
    ...
    # Load trained models
    pipeline = FastSRNeRF()
    pipeline.nerf.load_state_dict(torch.load("checkpoints/trained_nerf_weights.pth"))
    pipeline.sr.load_state_dict(torch.load("checkpoints/trained_edsr_weights.pth"))
    pipeline.cuda().eval()

    # Initialize PostProcessor with desired bounding box and resolution
    # Adjust bounding box and resolution depending on the scene scale.
